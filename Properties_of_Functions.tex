...
Find the value of each given the piecewise function below. Round to 3 decimal places where applicable.
\[
f(x) = \begin{cases}
|-3x-5| &\quad\text{if } x \leq -2 \\
5e^{2x+1} &\quad\text{if } -2 < x < 1 \\
\log_2\left(x^2 - 3x + 4\right) &\quad\text{if } 1 \leq x \leq 4 \\
-3\sin(3\pi x) + 7 &\quad\text{if } x > 4\\
\end{cases}
\]
\begin{multicols}{5}
\begin{enumerate}   \setcounter{enumi}{\value{Review}}
...
Find each of the following given the graph of $g(x)$ below.
\begin{center}
\begin{tikzpicture}
\begin{axis}
[axis lines = middle, xmin = -4, xmax = 4, ymin = -5.5, ymax = 5.5, ticks = none]
\addplot [<-, blue, very thick, domain=-3.5:3.5, samples=200] {0.5*x^2*cos(deg(2*x-pi/2))-1};
\addplot [blue, thick, mark = o, fill=white] coordinates {(3.5, 3.15)};
\addplot [blue, mark = *, only marks] coordinates {
(-2.543,2.01) (-1.144,-1.493) (1.144, -0.507) (2.543,-4.01) 
};
\node at (axis cs:-2.543,2.01) [above] {$(-2.54,2.01)$};
\node at (axis cs:-1.144,-1.493) [below, xshift=-0.2cm] {$(-1.14,-1.49)$};
\node at (axis cs:1.144, -0.507) [above, yshift=0.5cm] {$(1.14,-0.51)$};
\draw [->] (axis cs:1.14,0.6) -- (axis cs:1.14,-0.35);
\node at (axis cs:2.543,-4.01) [below] {$(2.54,-4.01)$};
\node at (axis cs:3.5,3.024) [above, left] {$(3.5,3.02)$};
\end{axis}
\end{tikzpicture}
\end{center}
